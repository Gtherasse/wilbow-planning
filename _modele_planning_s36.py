...
LIENS = {
    ...
    "603753": "https://wilbow.monday.com/boards/5089236279/pulses/3159304163",
...
# l'instant à côté de Miguel sur la ligne Équipe 2 -> son nom disparaît de
# l'entête tant qu'un renfort n'est pas désigné.
NOMS_OVERRIDE = {
...
PLANNING = {
    ...
    ("E2", 0): [J("575388", "Rue de Bruxelles 111 — Awans", "+ P. Mattiuz", "commun", caw="PANDA+")],
    ("E2", 1): [
        J("603753", "Rue Du Vicinal 17 à 31 — Clavier (Thomas et Piron)", "+ P. Mattiuz", "commun", caw="PANDA+"),
        J("599588", "Rue des Soeurs Grises/Montmorency/Av. du Condroz — Liège",
          note="Visite chantier", t="alt", caw="PANDA+"),
        J("682463", "Avenue du Condroz 3 — Huy (E.T.N.I.C.)",
          note="Visite chantier", t="alt", caw="PANDA+"),
    ],
    ("E2", 2): [
        J("600683", "Triererstrasse — Elsenborn (déplacement borne+ROP)", "+ P. Mattiuz", "commun", q="Matin", caw="PANDA+"),
        X("BTO", "+ P. Mattiuz", "commun", q="Après-midi", caw="BTO"),
    ],
    ("E2", 3): [J("582528", "RW SPW MI Pont d'Argenteau — Oupeye", "+ P. Mattiuz", "commun", caw="PANDA+")],
    ("E2", 4): [J("599588", "Rue des Soeurs Grises/Montmorency/Av. du Condroz — Liège", "+ P. Mattiuz", "commun", caw="PANDA+")],
    # échange avec lundi 07/09 (S37) : le 582528 y est mis à la place du 599588

    # --- JOINTEURS ---
    ("JM", 0): [
        J("590458", "FQ ch. de Wavre 36 — Héron (réparation forage dirigé)",
          note="PW", t="alt", q="Matin", caw="PANDA+"),
        J("667324", "Rue Sabaré — Visé (BBV)", caw="PANDA+"),
    ],
    ("JM", 1): [J("575388", "Rue de Bruxelles 111 — Awans", caw="PANDA+")],
    ("JM", 2): ABS("MALADIE"),   # Meurisse Johan malade mercredi 02/09 -> planning décalé d'1 jour
    ("JM", 3): ABS("MALADIE"),   # Meurisse Johan malade aussi jeudi 03/09 -> re-décalé d'1 jour
    ("JM", 4): ABS("MALADIE"),   # Meurisse Johan malade aussi vendredi 04/09 -> re-re-décalé d'1 jour
    # 575388 décalé sur lundi 07/09, 534240+667324 sur mardi 08/09, 672861 sur mercredi 09/09 (S37)

    ("PM", 0): [
        J("590458", "FQ ch. de Wavre 36 — Héron (réparation forage dirigé)",
          note="PW", t="alt", q="Matin", caw="PANDA+"),
        J("575388", "Rue de Bruxelles 111 — Awans", "+ Équipe 2", "commun", q="Après-midi", caw="PANDA+"),
    ],
    ("PM", 1): [J("603753", "Rue Du Vicinal 17 à 31 — Clavier (Thomas et Piron)", "+ Équipe 2", "commun", caw="PANDA+")],
    ("PM", 2): [
        J("600683", "Triererstrasse — Elsenborn (déplacement borne+ROP)", "+ Équipe 2", "commun", q="Matin", caw="PANDA+"),
        X("BTO", "+ Équipe 2", "commun", q="Après-midi", caw="BTO"),
    ],
    ("PM", 3): [J("582528", "RW SPW MI Pont d'Argenteau — Oupeye", "+ Équipe 2", "commun", caw="PANDA+")],
    ("PM", 4): [J("599588", "Rue des Soeurs Grises/Montmorency/Av. du Condroz — Liège", "+ Équipe 2", "commun", caw="PANDA+")],

    ("DP", 0): [X("BTO", caw="BTO")],
    ("DP", 1): [X("BTO", caw="BTO")],
    ("DP", 2): ABS("ABSENT"),   # Panait Dan absent mercredi 02/09
    ("DP", 3): ABS("ABSENT"),   # Panait Dan absent jeudi 03/09
    ("DP", 4): ABS("ABSENT"),   # Panait Dan absent vendredi 04/09
...
def rendre(rows, chemin, badge, soustitre, pied2, cle_vide):
    def pages(h):
        open("/tmp/_pl36.html", "w", encoding="utf-8").write(
            build(h, rows, badge, soustitre, pied2))
        return len(W(filename="/tmp/_pl36.html").render().pages)
    lo, hi = 4.0, 40.0
    if pages(lo) > 1:
    ...
    W(filename="/tmp/_pl36.html").write_pdf(chemin)
    print(f"OK -> {chemin}  (hauteur ligne {lo:.1f}mm)")

out_chantier = sys.argv[1] if len(sys.argv) > 1 else "planning_s36.pdf"
out_caw      = sys.argv[2] if len(sys.argv) > 2 else "planning_s36_caw.pdf"
