...
# ----------------------------------------------------------------- RESSOURCES
# Noville Yves ne travaille plus chez Wilbow depuis le 18/08/2026 -> retiré
# definitivement des ressources a partir de cette semaine (S35).
RESSOURCES = [
...
LIENS = {
    ...
    "679363": "https://wilbow.monday.com/boards/5089236279/pulses/3157275187",
    "603753": "https://wilbow.monday.com/boards/5089236279/pulses/3159304163",
    "601454": "https://wilbow.monday.com/boards/5089236279/pulses/3007140405",
...
# Sam Feller absent lundi-mercredi : Johan Meurisse prend sa place à côté de Miguel
# sur la ligne Équipe 2 ces 3 jours-là (son nom remplace celui de Sam dans l'entête).
NOMS_OVERRIDE = {
    ("E2", 0): ["Dominguez Miguel", "Meurisse Johan"],
    ("E2", 1): ["Dominguez Miguel", "Meurisse Johan"],
    ("E2", 2): ["Dominguez Miguel", "Mattiuz Pierre"],
}
...
PLANNING = {
    ...
    ("E1", 4): [J("685230", "Remplacement 12641 WELK — DOL", "+ Équipe 2", "commun", caw="PANDA+")],

    ("E2", 0): [X("BTO", caw="BTO")],
    ("E2", 1): [J("668465", "Bovigny 105 — Gouvy", caw="PANDA+")],
    ("E2", 2): [X("BTO", caw="BTO")],
    ("E2", 3): [J("679363", "Voie de l'univers A1.2 Neolegia — Liège", caw="PANDA+")],
    ("E2", 4): [J("685230", "Remplacement 12641 WELK — DOL", "+ Équipe 1", "commun", caw="PANDA+")],

    # --- JOINTEURS ---
    ("JM", 0): [X("BTO", "+ M. Dominguez", "commun", caw="BTO")],
    ("JM", 1): [J("668465", "Bovigny 105 — Gouvy", "+ M. Dominguez & P. Mattiuz", "commun", caw="PANDA+")],
    ("JM", 2): [X("Francorchamps", caw="CIRCUIT FANCORCHAMPS")],
    ("JM", 3): [J("535147", "FEED OUT FOx FH44/FH45 SB — Liège", caw="PANDA+")],
    ("JM", 4): [J("601454", "Sterpigny — Gouvy (D. Grandjean)", caw="PANDA+")],

    ("PM", 0): [J("654898", "Rue Pierre Joseph Antoine — Herstal (Embuild)", caw="PANDA+")],
    ("PM", 1): [J("668465", "Bovigny 105 — Gouvy", "+ M. Dominguez & J. Meurisse", "commun", caw="PANDA+")],
    ("PM", 2): [X("BTO", "+ M. Dominguez", "commun", caw="BTO")],
    ("PM", 3): [J("668465", "Bovigny 105 — Gouvy", caw="PANDA+")],
    ("PM", 4): [J("668465", "Bovigny 105 — Gouvy", caw="PANDA+")],
    ...
    # --- Absences confirmées (rappelées par Geoffrey) ---
    ("DP", 0): ABS("ABSENT"),   # Panait Dan absent lundi 24/08 -> BTO le reste de la semaine
    ("GA", 0): ABS("ABSENT"),   # Avenia Geoffrey absent lundi 24/08

    ("GA", 3): [J("601454", "Sterpigny — Gouvy (D. Grandjean)", "+ Fabrice Gaspard", caw="PANDA+")],
    ("GA", 4): [X("BTO", "+ Fabrice Gaspard", caw="BTO")],
...
def rendre(rows, chemin, badge, soustitre, pied2, cle_vide):
    def pages(h):
        open("/tmp/_pl35.html", "w", encoding="utf-8").write(
            build(h, rows, badge, soustitre, pied2))
        return len(W(filename="/tmp/_pl35.html").render().pages)
    lo, hi = 4.0, 40.0
    if pages(lo) > 1:
    ...
    W(filename="/tmp/_pl35.html").write_pdf(chemin)
    print(f"OK -> {chemin}  (hauteur ligne {lo:.1f}mm)")

out_chantier = sys.argv[1] if len(sys.argv) > 1 else "planning_s35.pdf"
out_caw      = sys.argv[2] if len(sys.argv) > 2 else "planning_s35_caw.pdf"
