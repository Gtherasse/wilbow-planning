...
# ------------------------------------------------------------------- DONNEES
SEMAINE = 33
LUNDI = datetime.date(2026, 8, 10)
# Horodatage automatique : date + heure de generation (heure belge)
MAJ = datetime.datetime.now(ZoneInfo("Europe/Brussels"))

J = lambda jms, lib, note=None, t=None, url=None, q=None: dict(jms=jms, lib=lib, note=note, t=t, url=url, q=q)
X = lambda lib, note=None, t=None, url=None, q=None, sub=None: dict(jms=None, lib=lib, note=note, t=t, url=url, q=q, sub=sub)
...
LIENS = {
    ...
    "640728": "https://wilbow.monday.com/boards/5089236279/pulses/2770267550",
    # 548528 = reference secondaire de l item "642445 - IS MP Creation NENA YVOIR - DINANT"
    "548528": "https://wilbow.monday.com/boards/5089236279/pulses/2687649138",
    "668454": "https://wilbow.monday.com/boards/5089236279/pulses/2987857365",
...
PLANNING = {
    # --- SOUFFLAGE ---
    ("E1", 0): [J("685230", "Remplacement 12641 WELK — DOL")],
    ("E1", 1): [J("685230", "Remplacement 12641 WELK — DOL")],
    ("E1", 2): [J("685230", "Remplacement 12641 WELK — DOL")],
    ("E1", 3): [J("652658", "Rue du Ruisseau 28-26-24 — Donceel")],
    ("E1", 4): [J("654898", "Rue Pierre Joseph Antoine — Herstal (Embuild)")],

    ("E2", 0): [X("BTO — Tirage câble aérien")],
    ("E2", 1): [J("668454", "Cherain 33a — Gouvy")],
    ("E2", 2): [X("ROP BTV", "Provisoire", "prov", url=ROP,
                  sub="Sam Feller · Miguel : après-midi")],
    ("E2", 3): [J("682102", "Grand'Route 100A-100B — Crisnée")],
    ("E2", 4): [J("682102", "Grand'Route 100A-100B — Crisnée")],

    # --- JOINTEURS ---
    ("JM", 0): [J("548528", "Nena Yvoir — Dinant")],
    ("JM", 1): [X("Fexhe")],
    ("JM", 2): [J("682102", "Grand'Route 100A-100B — Crisnée", "+ P. Mattiuz", "commun")],
    ("JM", 3): [X("Caterpillar Gensets", "+ P. Mattiuz & M. Wilvers", "commun", url=CAT)],
    ("JM", 4): [X("Caterpillar Gensets", "+ P. Mattiuz & M. Wilvers", "commun", url=CAT)],

    ("YN", 0): [J("640728", "Fond du Wa 2A+B+C — Remicourt", "+ P. Mattiuz", "commun")],
    ("YN", 1): [X("FH 32 FEED-IN", url=FH32)],
    ("YN", 2): [X("BTO")],
    ("YN", 3): [J("682102", "Grand'Route 100A-100B — Crisnée", "Jointage · + Éq. 2", "commun")],
    ("YN", 4): [J("652658", "Rue du Ruisseau 28-26-24 — Donceel", "Jointage", "commun")],

    ("PM", 0): [J("640728", "Fond du Wa 2A+B+C — Remicourt", "+ Y. Noville", "commun")],
    ("PM", 1): [J("668454", "Cherain 33a — Gouvy")],
    ("PM", 2): [J("682102", "Grand'Route 100A-100B — Crisnée", "+ J. Meurisse", "commun")],
    ("PM", 3): [X("Caterpillar Gensets", "+ J. Meurisse & M. Wilvers", "commun", url=CAT)],
    ("PM", 4): [X("Caterpillar Gensets", "+ J. Meurisse & M. Wilvers", "commun", url=CAT)],

    ("DP", 0): [A("ABSENT", JOUR),
                X("SignaDyn — Zone D", "+ F. Hanikenne", "commun", url=SIGN, q=NUIT)],
    ("DP", 1): ABS("ABSENT"),
    ("DP", 2): [X("SignaDyn — Zone D", "+ F. Hanikenne", "commun", url=SIGN, q=MATIN),
                X("BTO", q=AM)],
    ("DP", 3): [X("BTO", "+ F. Hanikenne", "commun")],
    ("DP", 4): [X("BTO", "+ F. Hanikenne", "commun")],

    ("FH", 0): [X("Fexhe", q=JOUR),
                X("SignaDyn — Zone D", "+ D. Panait", "commun", url=SIGN, q=NUIT)],
    ("FH", 1): ABS("RÉCUP"),
    ("FH", 2): [X("SignaDyn — Zone D", "+ D. Panait", "commun", url=SIGN, q=MATIN),
                X("BTO", q=AM)],
    ("FH", 3): [X("BTO", "+ D. Panait", "commun")],
    ("FH", 4): [X("BTO", "+ D. Panait", "commun")],

    ("GA", 3): [X("Soufflage µtubes", "+ F. Gaspard", "commun")],
    # GA : dépôt les autres jours (valeur par défaut)
}
...
# C = raccourci : affiche le code en titre et l intitule en dessous
def C(nom, note=None, t=None, q=None):
    # intitule en gras au-dessus, code en dessous
    return X(nom, note, t, q=q, sub=CODES[nom])

# ------------------------------------------------- CHECK IN @ WORK (secretaire)
# Meme structure que PLANNING, mais on indique la declaration C@W a encoder.
CAW = {
    ("E1", 0): [C("PANDA+")],
    ("E1", 1): [C("PANDA+")],
    ("E1", 2): [C("PANDA+")],
    ...
    ("E2", 0): [C("BTO")],
    ("E2", 1): [C("PANDA+")],
    ("E2", 2): [C("PANDA+", "M. Dominguez : matin absent", "alt")],
    ("E2", 3): [C("PANDA+")],
    ("E2", 4): [C("PANDA+")],
    ...
    ("DP", 0): [C("EQUANS-CAMERA-CCTV", "Journée : absent", "alt")],
    ("DP", 1): ABS("ABSENT"),
    ("DP", 2): [C("EQUANS-CAMERA-CCTV", q=MATIN), C("BTO", q=AM)],
    ("DP", 3): [C("BTO")],
    ("DP", 4): [C("BTO")],

    ("FH", 0): [C("EQUANS-CAMERA-CCTV")],
    ("FH", 1): ABS("RÉCUP"),
    ("FH", 2): [C("EQUANS-CAMERA-CCTV", q=MATIN), C("BTO", q=AM)],
...
def cellule(r, i, DATA=None):
    ...
    return (f'<td style="border-left-color:{r["c"]}; background:{r["bg"]};">'
...
def make_rows(DATA):
    return "".join(
        ...
        + "".join(cellule(r, i, DATA) for i in range(NB_JOURS)) + "</tr>"
        for r in RESSOURCES)
...
def rendre(rows, chemin, badge, soustitre, pied2, cle_vide):
    def pages(h):
        open("/tmp/_pl.html", "w", encoding="utf-8").write(
            build(h, rows, badge, soustitre, pied2))
        return len(W(filename="/tmp/_pl.html").render().pages)
    lo, hi = 4.0, 40.0
    if pages(lo) > 1:
    ...
    W(filename="/tmp/_pl.html").write_pdf(chemin)
    print(f"OK -> {chemin}  (hauteur ligne {lo:.1f}mm)")

out_chantier = sys.argv[1] if len(sys.argv) > 1 else "planning.pdf"
out_caw      = sys.argv[2] if len(sys.argv) > 2 else "planning_caw.pdf"

rendre(make_rows(PLANNING), out_chantier, "PLANNING CONFIRMÉ", None, None, True)
rendre(make_rows(CAW), out_caw, "CHECK IN @ WORK",
       " — CHECK IN @ WORK",
